...
class UserController extends Controller
{
    ...
    public function updateProfile(Request $request)
    {
        ...
        // For regular users, only allow password and avatar changes
        if ($user->isUser()) {
            ...
            // For admins and supervisors, allow all fields
            $validator = \Validator::make($request->all(), [
        ...
        // For regular users, only update avatar and password
        if ($user->isUser()) {
            // Update avatar if provided
            if ($avatarPath) {
                $userInfo = $user->userInfo;
                if (!$userInfo) {
                    $userInfo = $user->userInfo()->create(['avatar' => $avatarPath]);
                } else {
                    $userInfo->update(['avatar' => $avatarPath]);
                }
            }
        } else {
            // For admins and supervisors, update all fields
